...
public class ControllerValidationErrorHandling {

  @ResponseStatus(REQUEST_TIMEOUT)
  @ExceptionHandler(ConversionFailedException.class)
  public ApiResponseModel<?> timeFailHandle(ConversionFailedException e) {
    log.debug("convert fail", e);
    return ApiResponseModel.builder().success(false).message("Time Out.").build();
  }

  @ResponseStatus(BAD_REQUEST)
  @ExceptionHandler(Exception.class)
  public ApiResponseModel<?> convertFailHandle(Exception e) {
    log.debug("convert fail", e);
  ...
  private ApiResponseModel<?> buildResponse(BindingResult bindingResult) {
    ...
    for (int i = 0, size = errors.size(); i < size; i++) {
      ...
      if (payloads.contains(ClientNotification.class)) {
        clientMessage.append(errors.get(i).getDefaultMessage()).append(appendLine);
      }
